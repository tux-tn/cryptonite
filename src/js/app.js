...
export default class App {
  ...
  init() {
    ...
    this._socket.on('user:joined', (data) => {
      this._darkwire.decode(data).then((data) => {
        // expected 1
        ...
        this._darkwire.addUser(data.user).then((users) => {
          debugger;
          if (users.length <= 1) {
            $('#first-modal').modal('show');
          }

          this._chat.log(data.user.username + ' joined');
  ...
  initChat() {
    ...
    this._darkwire.createUser(username).then((user) => {
      this._darkwire.encode(user).then((socketData) => {
  ...
  renderParticipantsList() {
    ...
    $('#participants').text(this._darkwire.users.length);
